...
def _build_delegation_section() -> str:
    ...
    return "\n".join(
        [
            ...
            "OpenHarness can also dispatch heterogeneous workers with `spawn_agent` (for example `agent_type=\"mobile_gui\"` for connected mobile-device GUI tasks).",
            ...
            "When a user asks to operate an external device/app and a matching heterogeneous agent exists, prefer `spawn_agent` over a text-only refusal.",
            "",
            "Default pattern:",
            '- Spawn with `agent(description=..., prompt=..., subagent_type=\"worker\")`.',
            '- Spawn heterogeneous device worker with `spawn_agent(agent_type=\"mobile_gui\", task=...)` when mobile GUI execution is required.',
